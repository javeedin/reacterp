// Payment attachments panel — mirror of InvoiceAttachments.tsx, keyed by
// the payment's CHECK_ID. Same upload/download/delete UX as invoices.
...
import {
  listPaymentAttachments, uploadPaymentAttachment, downloadPaymentAttachment, deletePaymentAttachment,
  formatFileSize, fileIcon,
  type PaymentAttachment,
} from '../services/paymentAttachment.service';
...
const PaymentAttachments: React.FC<Props> = ({ checkId, readOnly = false, onCountChange }) => {
  const { user } = useAuth();
  ...
  const load = useCallback(async () => {
    if (checkId === null || checkId === undefined) return;
    setLoading(true);
    setFetchError(null);
    try {
      const rows = await listPaymentAttachments(checkId);
      setAttachments(rows);
  ...
  const handleUpload: UploadProps['customRequest'] = async (options) => {
    ...
    const result = await uploadPaymentAttachment(
      checkId!,
      file,
  ...
  // ── Download ─────────────────────────────────────────────────────────────
  const handleDownload = async (rec: PaymentAttachment) => {
    setDownloading(prev => new Set([...prev, rec.attachmentId]));
    try {
      await downloadPaymentAttachment(checkId!, rec.attachmentId, rec.fileName);
    } catch (e: any) {
  ...
  // ── Delete ───────────────────────────────────────────────────────────────
  const handleDelete = async (rec: PaymentAttachment) => {
    setDeleting(prev => new Set([...prev, rec.attachmentId]));
    const result = await deletePaymentAttachment(
      checkId!,
      rec.attachmentId,
      user?.name || user?.username || 'System',
    );
  ...
  // ── Table columns ─────────────────────────────────────────────────────────
  const columns: ColumnsType<PaymentAttachment> = [
    {
      title: '',
      width: 30,
      render: (_, rec) => (
  ...
  if (checkId === null || checkId === undefined) {
    return (
      <Alert
        type="info"
        showIcon
        message="Save the payment first to enable attachments"
        style={{ margin: '12px 0' }}
      />
    );
  }
