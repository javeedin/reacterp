...
// Types
export interface BankAccountsSyncProgress {
...
export type ProgressCallback = (progress: Partial<BankAccountsSyncProgress>) => void;
export type BankAccountsPayloadCallback = (
...
// Test connection to Oracle Fusion cashBankAccounts endpoint
export const testBankAccountsConnection = async (log: LogCallback): Promise<boolean> => {
  try {
    log('info', 'Testing Bank Accounts endpoint...');
...
export const syncBankAccounts = async (
  ...
  signal?: AbortSignal,
  onPayload?: BankAccountsPayloadCallback
): Promise<BankAccountsSyncProgress> => {
  const progress: BankAccountsSyncProgress = {
    status: 'fetching',
    totalRecords: 0,
    processedRecords: 0,
    insertedRecords: 0,
  ...
  log('step', '═══════════════════════════════════════════════════════════');
  log('step', `  BANK ACCOUNTS SYNC - ${modeLabel}`);
  log('step', '═══════════════════════════════════════════════════════════');
  ...
  try {
    ...
    while (hasMore && (maxRecords === null || allRecords.length < maxRecords)) {
      ...
      const result = await fetchFromOracle('cashBankAccounts', queryParams, log, true);
    ...
    for (let i = 0; i < allRecords.length; i += batchSize) {
      ...
      const payload = {
        items: batch.map(item => ({
          ...
          BankAccountNameAlt: item.BankAccountNameAlt,
          BankAccountNumber: item.BankAccountNumber,
          BankAccountNumberElectronic: item.BankAccountNumberElectronic,
          IBANNumber: item.IBANNumber,
          MaskedAccountNumber: item.MaskedAccountNumber,
          MaskedIBAN: item.MaskedIBAN,
          AccountHolderName: item.AccountHolderName,
          AccountHolderNameAlt: item.AccountHolderNameAlt,
          AccountSuffix: item.AccountSuffix,
          AccountType: item.AccountType,
          CurrencyCode: item.CurrencyCode,
          Description: item.Description,
          CheckDigits: item.CheckDigits,
          ...
          ApUseAllowedFlag: item.ApUseAllowedFlag,
          ArUseAllowedFlag: item.ArUseAllowedFlag,
          PayUseAllowedFlag: item.PayUseAllowedFlag,
          DataSecurityFlag: item.DataSecurityFlag,
          MultiCurrencyAllowedFlag: item.MultiCurrencyAllowedFlag,
          NettingAccountFlag: item.NettingAccountFlag,
          PooledFlag: item.PooledFlag,
          ZeroAmountAllowed: item.ZeroAmountAllowed,
          MultiCashReconEnabledFlag: item.MultiCashReconEnabledFlag,
          MinimumCheckAmount: item.MinimumCheckAmount,
          MaximumCheckAmount: item.MaximumCheckAmount,
          MaximumOutlay: item.MaximumOutlay,
          TargetBalance: item.TargetBalance,
          CashAccountCombination: item.CashAccountCombination,
          CashClearingAccountCombination: item.CashClearingAccountCombination,
          ReconciliationDifferenceAccountCombination: item.ReconciliationDifferenceAccountCombination,
          CashCcidFixedSegments: item.CashCcidFixedSegments,
          EndDate: item.EndDate,
          ReconStartDate: item.ReconStartDate,
          GlReconStartDate: item.GlReconStartDate,
          AgencyLocationCode: item.AgencyLocationCode,
          EFTUserNumber: item.EFTUserNumber,
          SecondaryAccountReference: item.SecondaryAccountReference,
          ReversalProcessingMethod: item.ReversalProcessingMethod,
          RulesetName: item.RulesetName,
          ToleranceRuleName: item.ToleranceRuleName,
          ParsingRuleSetName: item.ParsingRuleSetName,
          BankExchangeRateType: item.BankExchangeRateType,
          AccountingConversionRateType: item.AccountingConversionRateType,
          TransactionCalendar: item.TransactionCalendar,
          BankCompanyIdentifier: item.BankCompanyIdentifier,
      ...
      try {
        ...
        if (result.status === 'success') {
          ...
          if (onPayload) {
            batch.forEach(item => {
              onPayload(item.BankAccountId, item.BankAccountName, item, result);
            });
          }
        } else {
          progress.errors++;
          ...
          if (onPayload) {
            batch.forEach(item => {
              onPayload(item.BankAccountId, item.BankAccountName, item, result, result.message);
            });
          }
        }
      } catch (error) {
        ...
        if (onPayload) {
          batch.forEach(item => {
            onPayload(item.BankAccountId, item.BankAccountName, item, undefined, progress.lastError);
          });
        }
      }
