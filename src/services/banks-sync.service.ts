...
// Types
export interface BanksSyncProgress {
...
export type ProgressCallback = (progress: Partial<BanksSyncProgress>) => void;
export type BanksPayloadCallback = (
  bankPartyId: number,
  bankName: string,
  payload: any,
  result?: any,
...
export const syncBanks = async (
  ...
  signal?: AbortSignal,
  onPayload?: BanksPayloadCallback
): Promise<BanksSyncProgress> => {
  const progress: BanksSyncProgress = {
    status: 'fetching',
    totalRecords: 0,
    processedRecords: 0,
    insertedRecords: 0,
    ...
    for (let i = 0; i < allRecords.length; i += batchSize) {
      ...
      const payload = {
        items: batch.map(item => ({
          BankPartyId: item.BankPartyId,
          BankName: item.BankName,
          BankNameAlt: item.BankNameAlt,
          BankNumber: item.BankNumber,
          Description: item.Description,
          TaxpayerIdNumber: item.TaxpayerIdNumber,
      ...
      try {
        ...
        if (result.status === 'success') {
          ...
          if (onPayload) {
            batch.forEach(item => {
              onPayload(item.BankPartyId, item.BankName, item, result);
            });
          }
        } else {
          progress.errors++;
          ...
          if (onPayload) {
            batch.forEach(item => {
              onPayload(item.BankPartyId, item.BankName, item, result, result.message);
            });
          }
        }
      } catch (error) {
        ...
        if (onPayload) {
          batch.forEach(item => {
            onPayload(item.BankPartyId, item.BankName, item, undefined, progress.lastError);
          });
        }
      }
