-- ============================================================
-- Petty Cash Package — Fix ORA-01858
-- Adds parse_number helper; all GET_NUMBER calls routed
-- through VARCHAR2 + parse_number to prevent conversion errors
-- ============================================================
...
CREATE OR REPLACE PACKAGE BODY RR_PC_PKG AS
    ...
    -- ──────────────────────────────────────────────────────────
    -- parse_date: accepts YYYY-MM-DD or ISO timestamp, returns DATE
    -- Strips anything after position 10 (e.g. T00:00:00.000Z)
    -- ──────────────────────────────────────────────────────────
    ...
    -- ──────────────────────────────────────────────────────────
    -- parse_number: safely converts a string to NUMBER
    -- Returns NULL on any conversion error (prevents ORA-01858)
    -- ──────────────────────────────────────────────────────────
    ...
    BEGIN
        ...
        l_ccid     := parse_number(APEX_JSON.GET_VARCHAR2(p_path => 'cashAccountCcid'));
        ...
        l_limit    := parse_number(APEX_JSON.GET_VARCHAR2(p_path => 'limit'));
        l_by       := APEX_JSON.GET_VARCHAR2(p_path => 'createdBy');

        IF l_name IS NULL THEN
            p_error := 'registerName is required'; RETURN;
        END IF;
        IF l_bu IS NULL THEN
            p_error := 'businessUnit is required'; RETURN;
        END IF;
        IF l_status NOT IN ('DRAFT','ACTIVE') THEN
            p_error := 'status must be DRAFT or ACTIVE when creating a register'; RETURN;
        END IF;
    ...
    BEGIN
        ...
        l_reg_id := parse_number(APEX_JSON.GET_VARCHAR2(p_path => 'registerId'));
        ...
        l_ca_ccid   := parse_number(APEX_JSON.GET_VARCHAR2(p_path => 'chargeAccountCcid'));
        ...
        l_currency  := NVL(APEX_JSON.GET_VARCHAR2(p_path => 'currency'), 'AED');
        l_debit     := NVL(parse_number(APEX_JSON.GET_VARCHAR2(p_path => 'debitAmount')),   0);
        l_credit    := NVL(parse_number(APEX_JSON.GET_VARCHAR2(p_path => 'creditAmount')),  0);
        l_suspense  := NVL(parse_number(APEX_JSON.GET_VARCHAR2(p_path => 'suspenseAmount')), 0);
        l_orig_susp :=     parse_number(APEX_JSON.GET_VARCHAR2(p_path => 'originalSuspenseAmount'));
        l_comments  :=              APEX_JSON.GET_VARCHAR2(p_path => 'comments');
        ...
        l_by        :=              APEX_JSON.GET_VARCHAR2(p_path => 'createdBy');
        l_bank_txn_id := parse_number(APEX_JSON.GET_VARCHAR2(p_path => 'bankTxnId'));
    ...
    BEGIN
        ...
        l_currency     :=              APEX_JSON.GET_VARCHAR2(p_path => 'currency');
        l_debit        := parse_number(APEX_JSON.GET_VARCHAR2(p_path => 'debitAmount'));
        l_credit       := parse_number(APEX_JSON.GET_VARCHAR2(p_path => 'creditAmount'));
        l_suspense     := parse_number(APEX_JSON.GET_VARCHAR2(p_path => 'suspenseAmount'));
        l_comments     :=              APEX_JSON.GET_VARCHAR2(p_path => 'comments');
        l_ref_no       :=              APEX_JSON.GET_VARCHAR2(p_path => 'referenceNo');
        l_attach       :=              APEX_JSON.GET_VARCHAR2(p_path => 'attachment');
        l_attach_data  :=              APEX_JSON.GET_CLOB    (p_path => 'attachmentData');
