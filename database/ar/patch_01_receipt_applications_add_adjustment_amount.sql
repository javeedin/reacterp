-- =============================================================
-- PATCH 01 — RR_AR_RECEIPT_APPLICATIONS
-- Add ADJUSTMENT_AMOUNT column and update package to persist it.
...
BEGIN
    EXECUTE IMMEDIATE '
        ALTER TABLE RR_AR_RECEIPT_APPLICATIONS
        ADD (ADJUSTMENT_AMOUNT NUMBER(18,2))
    ';
    DBMS_OUTPUT.PUT_LINE('Column ADJUSTMENT_AMOUNT added.');
EXCEPTION
    WHEN OTHERS THEN
        IF SQLCODE = -01430 THEN   -- ORA-01430: column already exists
            DBMS_OUTPUT.PUT_LINE('Column ADJUSTMENT_AMOUNT already exists — skipped.');
        ELSE
            RAISE;
        END IF;
END;
...
COMMENT ON COLUMN RR_AR_RECEIPT_APPLICATIONS.ADJUSTMENT_AMOUNT
    IS 'Write-off / adjustment amount linked to this application row (not part of the cash applied).';
        ...
        FOR rec IN (
            ...
            BEGIN
                ...
                WHEN NOT MATCHED THEN INSERT (
                    ...
                    ADJUSTMENT_AMOUNT,           APPLICATION_STATUS,
                    ACCOUNTING_DATE,             REFERENCE_INSTALLMENT_ID,
                    REFERENCE_TRANSACTION_NUMBER, REFERENCE_TRANSACTION_ID,
                    REFERENCE_TRANSACTION_STATUS, ACTIVITY_NAME,
                    STANDARD_RECEIPT_ID,         RECEIPT_NUMBER,
                    ENTERED_CURRENCY,            RECEIPT_METHOD,
                    PROCESS_STATUS,              IS_LATEST_APPLICATION,
                    CUST_ACCOUNT_ID,             CUSTOMER_SITE,
                    ...
                    rec.ADJUSTMENT_AMOUNT,       rec.APPLICATION_STATUS,
                    l_acct_date,                 rec.REFERENCE_INSTALLMENT_ID,
                    rec.REFERENCE_TRANSACTION_NUMBER, rec.REFERENCE_TRANSACTION_ID,
                    rec.REFERENCE_TRANSACTION_STATUS, rec.ACTIVITY_NAME,
                    rec.STANDARD_RECEIPT_ID,     rec.RECEIPT_NUMBER,
                    rec.ENTERED_CURRENCY,        rec.RECEIPT_METHOD,
                    rec.PROCESS_STATUS,          rec.IS_LATEST_APPLICATION,
                    rec.CUST_ACCOUNT_ID,         rec.CUSTOMER_SITE,
...
SHOW ERRORS PACKAGE BODY RR_AR_RECEIPT_APPLICATIONS_PKG;
