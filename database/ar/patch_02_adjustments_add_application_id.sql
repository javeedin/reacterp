-- =============================================================
-- PATCH 02 — RR_AR_ADJUSTMENTS
-- Add APPLICATION_ID column (FK to RR_AR_RECEIPT_APPLICATIONS)
-- and update package to persist it.
-- Safe to re-run: uses EXECUTE IMMEDIATE with exception handling.
-- =============================================================
...
BEGIN
    ...
    ';
    DBMS_OUTPUT.PUT_LINE('Column APPLICATION_ID added.');
EXCEPTION
    WHEN OTHERS THEN
        IF SQLCODE = -01430 THEN   -- ORA-01430: column already exists
            DBMS_OUTPUT.PUT_LINE('Column APPLICATION_ID already exists — skipped.');
        ELSE
            RAISE;
        END IF;
END;
...
COMMENT ON COLUMN RR_AR_ADJUSTMENTS.APPLICATION_ID
    IS 'Links this adjustment to its parent receipt application in RR_AR_RECEIPT_APPLICATIONS.';

-- ── 2. Index for fast lookup by application ─────────────────
BEGIN
    EXECUTE IMMEDIATE '
        CREATE INDEX IDX_AR_ADJ_APP_ID
        ON RR_AR_ADJUSTMENTS (APPLICATION_ID)
    ';
    DBMS_OUTPUT.PUT_LINE('Index IDX_AR_ADJ_APP_ID created.');
EXCEPTION
    WHEN OTHERS THEN
        IF SQLCODE = -00955 THEN   -- ORA-00955: name already used by existing object
            DBMS_OUTPUT.PUT_LINE('Index IDX_AR_ADJ_APP_ID already exists — skipped.');
        ELSE
            RAISE;
        END IF;
END;
...
CREATE OR REPLACE PACKAGE BODY RR_AR_ADJUSTMENTS_PKG AS
    ...
    BEGIN
        IF p_str IS NULL OR TRIM(p_str) IS NULL THEN RETURN NULL; END IF;
        -- Handle ISO-8601 with timezone offset e.g. 2023-10-07T06:36:32+00:00
    ...
    BEGIN
        ...
        FOR rec IN (
            ...
            BEGIN
                ...
                USING DUAL
                ON (t.ADJUSTMENT_ID = rec.ADJUSTMENT_ID)
                ...
                WHEN NOT MATCHED THEN INSERT (
                    ...
                    INSTALLMENT_NUMBER,       INSTALLMENT_BALANCE,
                    ADJUSTMENT_REASON,        APPROVED_BY,
                    BILL_TO_SITE_USE_ID,      COMMENTS,
                    ACCOUNT_COMBINATION,      APPLICATION_ID,
                    FUSION_CREATED_BY,        FUSION_CREATION_DATE,
                    FUSION_LAST_UPDATED_BY,   FUSION_LAST_UPDATE_DATE,
                    SYNC_STATUS,              SYNC_DATE
                ) VALUES (
                    rec.ADJUSTMENT_ID,        rec.ADJUSTMENT_NUMBER,
                    ...
                    rec.INSTALLMENT_NUMBER,   rec.INSTALLMENT_BALANCE,
                    rec.ADJUSTMENT_REASON,    rec.APPROVED_BY,
                    rec.BILL_TO_SITE_USE_ID,  rec.COMMENTS,
                    rec.ACCOUNT_COMBINATION,  rec.APPLICATION_ID,
