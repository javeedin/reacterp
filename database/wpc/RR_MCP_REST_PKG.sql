-- ============================================================
-- RR_MCP_REST_PKG - Oracle Package for MCP Server REST API
...
CREATE OR REPLACE PACKAGE BODY RR_MCP_REST_PKG AS
    ...
    BEGIN
        ...
        BEGIN
            v_server_id := TO_NUMBER(p_id);
        EXCEPTION WHEN OTHERS THEN
            p_result := '{"status":"error","message":"Invalid server ID"}';
            RETURN;
        END;
        ...
        FROM RR_MCP_SERVERS
        WHERE mcp_server_id = v_server_id;

        v_json := '{"status":"success","id":"' || v_server_id || '",' ||
    ...
    BEGIN
        ...
        IF UPPER(p_action) = 'CREATE' THEN
            ...
            p_result := '{"status":"success","id":"' || v_new_id || '","message":"Server created"}';

        ELSIF UPPER(p_action) = 'UPDATE' THEN
            -- Update existing server
            BEGIN
                v_server_id_num := TO_NUMBER(p_server_id);
            EXCEPTION WHEN OTHERS THEN
                p_result := '{"status":"error","message":"Invalid server ID"}';
                RETURN;
            END;
            ...
            WHERE mcp_server_id = v_server_id_num;
