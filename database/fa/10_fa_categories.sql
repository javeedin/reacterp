...
--   GET  fa/categories/:id/books (category books + account codes)
...
BEGIN
    ORDS.DEFINE_TEMPLATE(
        ...
        p_priority    => 0,
        p_etag_type   => 'HASH',
        p_comments    => 'FA: category book accounts'
    );
    COMMIT;
END;
/
...
BEGIN
    ORDS.DEFINE_HANDLER(
        ...
        p_source         => q'[
DECLARE
    -- Builds account display string from GL code combinations segments
    ...
    CURSOR c IS
        SELECT cb.CATEGORY_BOOK_ID, cb.BOOK_TYPE_CODE,
               bc.BOOK_TYPE_NAME, bc.BOOK_CLASS,
