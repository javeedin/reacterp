-- ============================================================
-- PATCH 121: RR_AP_MPA_PKG.list_schedules — add invoice amount + MPA dates
--
-- Problem:
--   GET /ap/multiperiod delegates to RR_AP_MPA_PKG.list_schedules, which only
--   returned "totalAmount" = SUM(schedule PERIOD_AMOUNT) (the MPA-scheduled
--   total). The Manage Multiperiod grid needs the full invoice header amount and
--   the invoice's MPA accrual window too.
--
-- Fix:
--   Recompile the package BODY (spec unchanged) so list_schedules also returns:
--     • invoiceAmount  — RR_AP_INVOICES_ALL.INVOICE_AMOUNT (full invoice total)
...
--   Everything else is byte-for-byte the currently deployed body. The existing
--   GET handler (which calls this package) needs no change.
--
...
create or replace PACKAGE BODY RR_AP_MPA_PKG AS
    ...
    FOR ln IN (
      ...
      WHILE v_cur_month <= v_last_month LOOP
        ...
        INSERT INTO RR_AP_INVOICE_MULTIPERIOD_SCHEDULE (
          ...
          POSTING_STATUS, TRANSACTION_STATUS,
          ...
          ln.DESCRIPTION, NVL(v_currency, 'AED'),
          'Not Posted', 'Active',
  ...
  -- ── list_schedules — with invoice amount + MPA start/end dates (patch 121) ─
  ...
  BEGIN
    ...
    FOR r IN (
      SELECT
        ...
        SUM(CASE WHEN s.POSTING_STATUS = 'Posted'     THEN 1 ELSE 0 END)               AS CLOSED_LINES,
        SUM(CASE WHEN s.POSTING_STATUS = 'Not Posted' THEN 1 ELSE 0 END)               AS OPEN_LINES,
     ...
     ORDER BY s.INVOICE_NUMBER
      ...
      app(v_buf,
          ...
          || '"mpaStartDate":'     || jdate(r.MPA_START_DATE)   || ','
          || '"mpaEndDate":'       || jdate(r.MPA_END_DATE)
          || '}'
      );
    END LOOP;
  ...
  BEGIN
    ...
    BEGIN
      SELECT INVOICE_NUMBER, SUPPLIER, BUSINESS_UNIT, INVOICE_DATE, CURRENCY_CODE
       ...
       WHERE INVOICE_ID = p_invoice_id AND ROWNUM = 1;
