-- ============================================================
-- PATCH 95: Add SYNC_STATUS and CREATED_BY to the
--           get_applied_by_invoice_snake response
--
-- Problem:
--   ap/invoices/appliedprepayments GET did not return SYNC_STATUS
--   or CREATED_BY, so the UI had no way to distinguish rows that
--   were synced from Oracle Fusion from locally created ones.
--
-- Fix:
--   Rebuild get_applied_by_invoice_snake inside
--   RR_AP_APPLIED_PREPAYMENTS_PKG to include:
--     "sync_status"  — 'SYNCED' (Fusion) or 'NEW' (local)
--     "created_by"   — audit column
--
-- UI behaviour driven by sync_status:
--   SYNCED → Accounting dropdown hidden, Un-Apply disabled
--   NEW    → full actions available
--
-- HOW TO RUN:
--   APEX SQL Workshop → SQL Commands — run this single block.
-- ============================================================
...
CREATE OR REPLACE PACKAGE BODY RR_AP_APPLIED_PREPAYMENTS_PKG AS
    ...
    BEGIN
        ...
        ELSE
            INSERT INTO RR_AP_APPLIED_PREPAYMENTS (
                APPLICATION_ID, INVOICE_ID, INVOICE_NUMBER,
                PREPAYMENT_INVOICE_ID, PREPAYMENT_NUMBER,
                LINE_NUMBER, PREPAYMENT_LINE_NUMBER, DESCRIPTION,
                BUSINESS_UNIT, SUPPLIER_SITE, PURCHASE_ORDER, CURRENCY,
                APPLIED_AMOUNT, INCLUDED_TAX, INCLUDED_ON_INVOICE_FLAG,
                APPLICATION_ACCOUNTING_DATE, STATUS,
                CREATED_BY, CREATION_DATE, LAST_UPDATED_BY,
                LAST_UPDATE_DATE, LAST_UPDATE_LOGIN,
                LOCAL_CREATED_DATE, LOCAL_UPDATED_DATE, SYNC_STATUS
            ) VALUES (
                v_application_id, v_invoice_id, v_invoice_number,
                v_prepayment_invoice_id, v_prepayment_number,
                v_line_number, v_prepayment_line_number, v_description,
                v_business_unit, v_supplier_site, v_purchase_order, v_currency,
                v_applied_amount, v_included_tax, v_included_on_invoice_flag,
                v_application_acct_date, v_status,
                v_created_by, v_creation_date, v_last_updated_by,
                v_last_update_date, v_last_update_login,
                SYSTIMESTAMP, SYSTIMESTAMP, 'NEW'
            );
        END IF;
        ...
        BEGIN
            ...
            BEGIN
                SELECT sm.SUPPLIER_NUMBER, ss.PREPAYMENT_ACCOUNT_ID, ss.LIABILITY_ACCOUNT_ID
                ...
                WHERE  inv.invoice_id = v_prepayment_invoice_id AND ROWNUM = 1;
                ...
                EXCEPTION WHEN OTHERS THEN v_prepay_account := 'PREPAYMENT-ACCOUNT'; END;
                ...
                EXCEPTION WHEN OTHERS THEN v_liab_account := 'LIABILITY-ACCOUNT'; END;
            EXCEPTION WHEN OTHERS THEN
            ...
            v_sla_body :=
                ...
                '"lines":[' ||
                    '{"lineNumber":1,"lineType":"DR","accountingClass":"LIABILITY",' ||
                     ...
                     '"enteredDr":' || v_amount || ',"enteredCr":0,' ||
                     '"accountedDr":' || v_amount || ',"accountedCr":0,' ||
                     '"currencyCode":"' || v_eff_currency || '",' ||
                    ...
                    '{"lineNumber":2,"lineType":"CR","accountingClass":"PREPAYMENT",' ||
                     ...
                     '"enteredDr":0,"enteredCr":' || v_amount || ',' ||
                     '"accountedDr":0,"accountedCr":' || v_amount || ',' ||
                     '"currencyCode":"' || v_eff_currency || '",' ||
    ...
    PROCEDURE save_bulk(p_json IN CLOB, p_result OUT VARCHAR2) IS
        v_count  NUMBER := 0;
        v_errors NUMBER := 0;
    ...
    BEGIN
        FOR rec IN (
            ...
            BEGIN
                ...
                IF v_single_result LIKE '%"status":"success"%' THEN v_count := v_count + 1;
        ...
        p_result := '{"status":"' || CASE WHEN v_errors = 0 THEN 'success' ELSE 'partial' END || '"'
                 || ',"saved":' || v_count || ',"errors":' || v_errors || '}';
    EXCEPTION
        WHEN OTHERS THEN
            p_result := '{"status":"error","message":"' || REPLACE(SQLERRM, '"', '\"') || '"}';
    END save_bulk;

    PROCEDURE save_from_items(p_json IN CLOB, p_result OUT VARCHAR2) IS
        v_items_json CLOB;
    BEGIN
    ...
    FUNCTION get_by_invoice_id(p_invoice_id IN NUMBER) RETURN CLOB IS
        v_result CLOB;
    BEGIN
        SELECT JSON_ARRAYAGG(
            JSON_OBJECT(
                ...
                'Status'                   VALUE status,
                'SyncStatus'               VALUE NVL(sync_status, 'NEW'),
    ...
    FUNCTION get_by_prepayment_id(p_prepayment_invoice_id IN NUMBER) RETURN CLOB IS
        v_result CLOB;
    BEGIN
        SELECT JSON_ARRAYAGG(
            JSON_OBJECT(
    ...
    FUNCTION get_available_by_supplier_id(p_supplier_id IN NUMBER) RETURN CLOB IS
        v_result        CLOB;
        v_supplier_num  VARCHAR2(30);
    BEGIN
        BEGIN
            SELECT supplier_number INTO v_supplier_num
            FROM   RR_SUPPLIER_MASTER WHERE supplier_id = p_supplier_id;
        EXCEPTION WHEN NO_DATA_FOUND THEN RETURN '{"items":[],"count":0}'; END;
        ...
        LEFT JOIN (
            SELECT prepayment_invoice_id,
                   SUM(CASE WHEN status != 'Cancelled' THEN applied_amount ELSE 0 END) AS total_applied
            FROM   RR_AP_APPLIED_PREPAYMENTS GROUP BY prepayment_invoice_id
        ) agg ON agg.prepayment_invoice_id = inv.invoice_id
        LEFT JOIN (
            SELECT invoice_id, MIN(purchase_order_number) AS purchase_order_number
            FROM   RR_AP_INVOICE_LINES_ALL WHERE purchase_order_number IS NOT NULL
            GROUP BY invoice_id
    ...
    -- ── get_applied_by_invoice_snake ─────────────────────────────────────────
    -- Returns snake_case JSON for the Pre-Payment Applications tab.
    -- Now includes sync_status and created_by so the UI can:
    --   • Show a "FUSION SYNC" badge when sync_status = 'SYNCED'
    --   • Disable Un-Apply and Accounting actions for those rows
    -- ─────────────────────────────────────────────────────────────────────────
    FUNCTION get_applied_by_invoice_snake(p_invoice_id IN NUMBER) RETURN CLOB IS
        v_result CLOB;
    BEGIN
        SELECT JSON_OBJECT(
            'items' VALUE JSON_ARRAYAGG(
