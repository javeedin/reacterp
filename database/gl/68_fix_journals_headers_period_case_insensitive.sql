-- ============================================================
-- PATCH 68: Fix gl/journals/headers — case-insensitive PERIOD_NAME
--           and LEDGER_NAME filters in search_journals_json
--
-- Problem:
--   The WHERE clause uses exact-match (=) for PERIOD_NAME and LEDGER_NAME:
--     AND (p_period IS NULL OR h.PERIOD_NAME = p_period)
--     AND h.LEDGER_NAME = p_ledger
--   URL parameter "period=May-26" does NOT match DB value "MAY-26"
--   (Oracle period names are stored uppercase).  Newly created journals
--   that store the period name in a different case are silently excluded.
--
-- Fix:
--   Wrap both sides in UPPER() so matching is case-insensitive.
--   Applied in BOTH the c_headers cursor and get_journal_count function.
--
-- HOW TO RUN:
--   APEX SQL Workshop → SQL Commands
--   Run the full CREATE OR REPLACE PACKAGE BODY block below.
-- ============================================================
