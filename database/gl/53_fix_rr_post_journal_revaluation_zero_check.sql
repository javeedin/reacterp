-- ============================================================
-- Fix RR_POST_JOURNAL: allow Revaluation journals where entered = 0
--
-- Revaluation journals (jeCategory = 'Revaluation') have:
--   ENTERED_DR / ENTERED_CR = 0  (no movement in foreign currency)
--   ACCOUNTED_DR / ACCOUNTED_CR  = AED functional currency amounts
--
-- The deployed procedure checks zero-amount lines using only
-- ENTERED_DR/CR, which flags every revaluation line as invalid.
--
-- Fixes applied:
--   1. Read JE_CATEGORY from the first journal header.
--   2. Zero check: Revaluation lines are invalid only when BOTH
--      entered AND accounted are all zero.
--   3. Balance check: Revaluation uses ACCOUNTED totals (entered
--      totals are always 0 for these journals).
-- ============================================================
...
DBMS_OUTPUT.PUT_LINE('RR_POST_JOURNAL updated — Revaluation category bypasses entered=0 zero check');
