-- ============================================================
-- Fix RR_INSERT_GL_JOURNALS_POST: populate DEFAULT_EFFECTIVE_DATE
-- on RR_GL_JE_HEADERS from header.defaultEffectiveDate in the
-- JSON payload.  Run in APEX SQL Workshop.
-- ============================================================
...
AS
    ...
    v_header_effective_date     DATE;            -- accounting date
    ...
    BEGIN
        IF p_obj.has(p_key) AND NOT p_obj.get(p_key).is_null() THEN
            ...
            IF l_date_str IS NOT NULL THEN
                RETURN TO_DATE(l_date_str, 'YYYY-MM-DD');
            END IF;
        END IF;
        RETURN NULL;
    EXCEPTION
...
BEGIN
    ...
    v_header_status          := NVL(safe_get_string(l_header_obj, 'status'), 'NEW');
    ...
    INSERT INTO RR_GL_JE_HEADERS (
        ...
        l_batch_id,
        v_header_ledger_id,
        v_header_ledger_name,
        v_header_je_category,
        v_header_je_source,
        v_header_period_name,
        v_header_journal_name,
        v_header_description,
        v_header_currency,
        v_header_conv_type,
        v_header_conv_date,
        v_header_conv_rate,
        v_header_effective_date,
        v_header_status,
        v_header_total_dr,
        v_header_total_cr,
        v_header_created_by,
    ...
    l_result_obj.put('message', 'GL Journal created successfully');
    l_result_obj.put('batchId', l_batch_id);
    l_result_obj.put('headerId', l_header_id);
    l_result_obj.put('linesInserted', l_line_count);
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        l_result_obj.put('errorCode', SQLCODE);
        p_json_output := l_result_obj.to_clob();
