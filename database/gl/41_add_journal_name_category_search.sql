-- ============================================================
-- Add journalName / journalNameOp and category search params.
-- Builds on file 40 — run AFTER 40_fix_journals_search_combined.sql.
-- Safe to re-run.
-- ============================================================

-- ============================================================
-- 1. PACKAGE SPEC — add p_journal_name / p_journal_name_op
--                       p_category to both procedures/functions
-- ============================================================
    ...
    PROCEDURE search_journals_json(
        ...
        CURSOR c_headers IS
            ...
            WHERE h.LEDGER_NAME = p_ledger
              ...
              -- Batch Name: search BATCH_NAME OR BATCH_DESCRIPTION
              AND (p_batch_name IS NULL
                   ...
                   OR UPPER(NVL(b.BATCH_DESCRIPTION,'')) LIKE CASE UPPER(NVL(p_batch_name_op,'C'))
                                                                 ...
                                                                 ELSE '%' || UPPER(p_batch_name) || '%'
                                                               END)
              -- Journal Name: search JOURNAL_NAME
...
-- ============================================================
-- 3. ORDS GET HANDLER — reerp module, gl/journals/headers
--    Adds :journalName, :journalNameOp, :category bind vars
-- ============================================================
BEGIN
    ORDS.DEFINE_TEMPLATE(
        ...
        p_priority    => 0,
        p_etag_type   => 'HASH',
        p_comments    => 'GL Journal Headers — search and sync'
    );
    COMMIT;
...
BEGIN
    ORDS.DEFINE_HANDLER(
        ...
        p_items_per_page => 0,
        p_comments       => 'Search journals: batchName/batchOp, journalName/journalNameOp, journalDesc/journalOp, category, date range',
        p_source         => q'[
DECLARE
...
BEGIN
    IF :ledger IS NULL THEN
        ...
        APEX_JSON.WRITE('error', 'ledger is mandatory');
        APEX_JSON.CLOSE_OBJECT;
        RETURN;
    END IF;
    ...
    IF :period IS NULL AND :from_date IS NULL AND :to_date IS NULL THEN
        ...
        APEX_JSON.OPEN_OBJECT;
        APEX_JSON.WRITE('success', FALSE);
        APEX_JSON.WRITE('error', 'period or from_date/to_date is required');
        APEX_JSON.CLOSE_OBJECT;
        RETURN;
    END IF;
    ...
    RR_MANAGE_JOURNALS_PKG.search_journals_json(
        ...
        p_journal_name    => :journalName,
        p_journal_name_op => NVL(:journalNameOp,   'C'),
...
END;
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('OK: GET reerp/gl/journals/headers updated with journalName + category support');
END;
/
