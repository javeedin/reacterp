-- ============================================================
-- ALL-IN-ONE FIX: journals search with operator support
-- Run this single file in APEX SQL Workshop.
-- Replaces files 37 + 38 + 39 (can be re-run safely).
--
-- Fixes:
--  1. Package spec  — updated signatures
--  2. Package body  — CASE-in-SQL operator logic (no variable risk)
--  3. ORDS handler  — reerp module, gl/journals/headers, GET
-- ============================================================
...
-- ============================================================
-- 2. PACKAGE BODY
--    Uses CASE directly inside the SQL cursor — no variable
--    reference needed, works in all Oracle PL/SQL versions.
-- ============================================================
...
BEGIN
    ORDS.DEFINE_HANDLER(
        ...
        p_items_per_page => 0,
        p_comments       => 'Search journals: batchName/batchOp + journalDesc/journalOp + date range',
        p_source         => q'[
DECLARE
...
END;
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('OK: GET reerp/gl/journals/headers registered');
END;
/
