-- ============================================================
-- PATCH 66: Fix banktxn-lines — join RR_GL_JE_HEADERS for dates
--
-- RR_GL_JE_HEADERS exists in the schema.  Patch 62 avoided it
-- because it was believed not to exist (ORA-00942 from patch 61).
-- Now that it is confirmed present, restore the JOIN so the
-- handler uses the correct accounting date fields:
--   ACCOUNTING_DATE      ← h.DEFAULT_EFFECTIVE_DATE (or h.ACCOUNTING_DATE)
--   PERIOD_NAME          ← h.PERIOD_NAME
--   JE_SOURCE            ← h.JE_SOURCE
--   JE_CATEGORY          ← h.JE_CATEGORY
--   LEDGER_NAME          ← h.LEDGER_NAME
--   JOURNAL_NAME         ← h.NAME
--
-- Date filters also use h.DEFAULT_EFFECTIVE_DATE.
--
-- HOW TO RUN:
...
BEGIN
    ...
    ORDS.DEFINE_HANDLER(
        ...
        p_items_per_page => 0,
        p_mimes_allowed  => '',
        p_comments       => 'GL lines for bank transfer recon — RR_GL_JE_LINES_ALL + RR_GL_JE_HEADERS + RR_GL_JOURNAL_BATCHES',
        p_source         => q'[
DECLARE
...
BEGIN
    ...
    FOR r IN (
        SELECT
            ...
            TO_CHAR(NVL(h.DEFAULT_EFFECTIVE_DATE, h.ACCOUNTING_DATE), 'YYYY-MM-DD')
                                                                  AS ACCOUNTING_DATE,
            h.PERIOD_NAME,
            h.JE_SOURCE,
            h.JE_CATEGORY,
        ...
        WHERE (l_bank_acct  IS NULL OR UPPER(l.REFERENCE7) LIKE '%' || UPPER(l_bank_acct)  || '%')
          ...
          AND (l_date_from  IS NULL OR NVL(h.DEFAULT_EFFECTIVE_DATE, h.ACCOUNTING_DATE) >= TO_DATE(l_date_from, 'YYYY-MM-DD'))
          AND (l_date_to    IS NULL OR NVL(h.DEFAULT_EFFECTIVE_DATE, h.ACCOUNTING_DATE) <  TO_DATE(l_date_to,   'YYYY-MM-DD') + 1)
          AND (l_reconciled IS NULL OR NVL(l.RECONCILED_FLAG, 'N') = l_reconciled)
        ORDER BY NVL(h.DEFAULT_EFFECTIVE_DATE, h.ACCOUNTING_DATE) DESC, l.JE_HEADER_ID DESC, l.JE_LINE_NUMBER
