-- ============================================================
-- PATCH 62: Fix banktxn-lines — remove JOIN to RR_GL_JE_HEADERS
--
-- Root cause:
--   Patch 61 STEP 2 joins RR_GL_JE_HEADERS which does not exist
--   in the schema (ORA-00942 at line 44 of the handler body).
--
-- Fix:
--   Rewrite the handler to join only RR_GL_JE_LINES_ALL +
--   RR_GL_JOURNAL_BATCHES, pulling header-level fields from the
--   batch row:
--     JOURNAL_NAME     ← b.BATCH_NAME
--     ACCOUNTING_DATE  ← b.CREATION_DATE  (closest available proxy)
--     PERIOD_NAME      ← b.ACCOUNTING_PERIOD
--     JE_SOURCE        ← b.USER_JE_SOURCE_NAME
--     LEDGER_NAME      ← b.LEDGER_NAME
--     JE_CATEGORY      ← NULL (not stored on batch)
--
-- HOW TO RUN:
...
BEGIN
    ...
    ORDS.DEFINE_HANDLER(
        ...
        p_items_per_page => 0,
        p_mimes_allowed  => '',
        p_comments       => 'GL lines for bank transfer recon — RR_GL_JE_LINES_ALL + RR_GL_JOURNAL_BATCHES (no RR_GL_JE_HEADERS)',
        p_source         => q'[
DECLARE
...
BEGIN
    ...
    FOR r IN (
        SELECT
            ...
            b.ACCOUNTING_PERIOD                                  AS PERIOD_NAME,
        ...
        WHERE (l_bank_acct  IS NULL OR UPPER(l.REFERENCE7) LIKE '%' || UPPER(l_bank_acct)  || '%')
          ...
          AND (l_date_from  IS NULL OR b.CREATION_DATE >= TO_DATE(l_date_from, 'YYYY-MM-DD'))
          AND (l_date_to    IS NULL OR b.CREATION_DATE <  TO_DATE(l_date_to,   'YYYY-MM-DD') + 1)
          AND (l_reconciled IS NULL OR NVL(l.RECONCILED_FLAG, 'N') = l_reconciled)
        ORDER BY b.CREATION_DATE DESC, l.JE_HEADER_ID DESC, l.JE_LINE_NUMBER
