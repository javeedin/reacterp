-- ============================================================
-- Fix RR_POST_JOURNAL: allow entered=0 lines for Revaluation journals
--
-- Oracle Fusion revaluation journals post with:
--   - Journal currency  = foreign currency (e.g. INR)
--   - Entered  DR/CR    = 0  (no movement in the foreign currency)
--   - Accounted DR/CR   = AED amounts (functional currency adjustment)
--   - Conversion rate   = new exchange rate
--
-- The existing zero-amount check (validation g) flags lines where
-- ENTERED_DR = 0 AND ENTERED_CR = 0, rejecting valid revaluation lines.
--
-- Fix applied:
--   g) For category = 'Revaluation': a line is zero only when BOTH
--      entered AND accounted amounts are all zero.
--   6) For category = 'Revaluation': balance check uses ACCOUNTED
--      totals (entered totals are always 0 for these journals).
-- ============================================================
...
BEGIN
    ...
    -- ── 2. Period name + JE category from the first journal header ────────────
    -- COALESCE handles both Fusion-synced rows (JE_CATEGORY) and locally-created
    -- rows (USER_JE_CATEGORY_NAME) so Revaluation check works for both sources.
    BEGIN
        SELECT PERIOD_NAME, NVL(COALESCE(JE_CATEGORY, USER_JE_CATEGORY_NAME), '')
    ...
    -- Zero-amount check:
    --   Revaluation journals: entered = 0, accounted = AED amounts (Oracle standard).
    --   A line is invalid only when BOTH entered AND accounted are all zero.
    --   All other categories: original strict check on entered amounts only.
    SELECT COUNT(*),
    ...
    -- ── 6. Balance check ─────────────────────────────────────────────────────
    -- Revaluation: entered totals are always 0 — use accounted totals instead.
    IF v_line_count > 0 THEN
        DECLARE
            v_chk_dr NUMBER;
            v_chk_cr NUMBER;
...
DBMS_OUTPUT.PUT_LINE('RR_POST_JOURNAL updated — Revaluation category allows entered=0 with accounted amounts');
