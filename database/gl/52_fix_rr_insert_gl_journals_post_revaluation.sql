-- ============================================================
-- Fix RR_INSERT_GL_JOURNALS_POST: honour explicit accountedDr/Cr
-- for Revaluation journals instead of always computing from entered × rate.
--
-- Oracle Fusion revaluation journals have:
--   enteredDr/Cr  = 0   (no movement in the foreign currency)
--   accountedDr/Cr = AED amounts (functional currency adjustment)
--
-- Previous behaviour always computed:
--   v_line_accounted_dr := ROUND(NVL(v_line_entered_dr, 0) * v_eff_rate, 2)
-- → For revaluation lines this gives 0 × rate = 0, losing the AED amounts.
--
-- Fix:
--   1. Read accountedDr / accountedCr from the JSON payload.
--   2. If jeCategory = 'Revaluation': use the explicit values;
--      fall back to entered × rate only when the JSON field is null.
--   3. All other categories: unchanged — always compute from entered × rate.
-- ============================================================
...
BEGIN
    ...
    FOR i IN 0 .. l_lines_arr.get_size() - 1 LOOP
        ...
        IF UPPER(NVL(v_header_je_category, '')) = 'REVALUATION' THEN
            -- Revaluation journals: enteredDr/Cr = 0 by Oracle design.
            -- The caller sends the real AED amounts in accountedDr/Cr — use them directly.
            -- Fall back to entered × rate only when the explicit field is absent/null.
            ...
            END IF;
        ELSE
            -- All other categories: always derive accounted from entered × effective rate.
            v_line_accounted_dr := ROUND(NVL(v_line_entered_dr, 0) * v_eff_rate, 2);
            v_line_accounted_cr := ROUND(NVL(v_line_entered_cr, 0) * v_eff_rate, 2);
        END IF;
...
DBMS_OUTPUT.PUT_LINE('RR_INSERT_GL_JOURNALS_POST updated — Revaluation journals now use explicit accountedDr/Cr from payload');
