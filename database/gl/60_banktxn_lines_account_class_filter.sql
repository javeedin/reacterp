-- ============================================================
-- PATCH 60: Add account_class filter to banktxn-lines endpoint
--
-- Purpose:
--   Without filtering by REFERENCE3 (accounting class), both lines
--   of a bank transfer journal (BANK_ASSET + CASH_CLEARING) are
--   returned when reference7 is set on both. This patch adds an
--   account_class param so the frontend can request only BANK_ASSET
--   lines — one line per bank account per transfer.
--
-- HOW TO RUN:
...
DECLARE
    ...
    l_acct_class  VARCHAR2(100) := :account_class;   -- e.g. BANK_ASSET
