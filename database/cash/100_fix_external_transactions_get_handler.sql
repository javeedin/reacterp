-- =============================================================================
-- PATCH 100: Fix GET /cash/externaltransactions handler
--
-- BUGS FIXED:
--   1. HTP.P added \n after every 32 KB chunk — newline injected inside JSON
--      strings that span the chunk boundary → "Unexpected non-whitespace
--      character after JSON".  Fix: HTP.PRN (no trailing newline).
--
--   2. TO_CHAR(AMOUNT) used the session NLS decimal separator.  If the DB
--      NLS_NUMERIC_CHARACTERS is ',.', amounts appeared as "1234,56" which
--      the JSON parser treated as two separate values.
--      Fix: explicit 'NLS_NUMERIC_CHARACTERS=''.,''' in every TO_CHAR(NUMBER).
--
--   3. p_source_type 'plsql/block' — PL/SQL compilation errors (e.g. missing
--      column) caused ORDS to return an HTML error page instead of the
--      EXCEPTION block's JSON response.
--      Fix: ORDS.source_type_plsql.
--
--   4. Response lacked a "total" field — pagination showed 0 total records.
--      Fix: add a COUNT(*) query before the main cursor.
--
-- HOW TO RUN:
...
BEGIN
    ...
    ORDS.DEFINE_HANDLER(
        ...
        p_items_per_page => 0,
        p_mimes_allowed  => '',
        p_comments       => 'Query external cash transactions (patch 100: HTP.PRN fix, NLS fix, total count)',
        p_source         => q'[
DECLARE
    ...
    BEGIN
        ...
        -- Strip other control chars CHR(0-7), CHR(11), CHR(14-31)
        v := REGEXP_REPLACE(v, '[' || CHR(0) || '-' || CHR(7) || CHR(11) || CHR(14) || '-' || CHR(31) || ']', '');
        RETURN '"' || v || '"';
    END jstr;

    -- Format a NUMBER as JSON-safe decimal string (always dot as decimal separator)
...
BEGIN
    ...
    -- Stream output in 32 KB chunks using HTP.PRN (no trailing newline per chunk)
    :status_code := 200;
...
END;
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('GET /cash/externaltransactions redeployed (patch 100: HTP.PRN + NLS fix + total count).');
END;
/
