-- ============================================================
-- Patch: Add RECONCILED_FLAG to RR_EXTERNAL_CASH_TRANSACTIONS
-- GET handler: add recon_status filter and reconciledFlag JSON field
-- POST/MERGE: reconciledFlag is already handled by the existing
--             MERGE procedure (field mapped via JSON_TABLE).
-- Run in Oracle APEX SQL Workshop (reerp module must exist)
-- ============================================================

-- ── Step 1: Add RECONCILED_FLAG column (idempotent) ──────────
BEGIN
  EXECUTE IMMEDIATE '
...
DECLARE
    ...
    l_recon_status VARCHAR2(30)  := :recon_status;   -- RECONCILED | UNRECONCILED
...
-- ── Step 3: Ensure MERGE procedure stores ReconciledFlag ─────
-- In your RR_SYNC_EXTERNAL_CASH_TRANSACTIONS (or equivalent POST handler),
...
--
-- The bank recon flow now POSTs only:
--   { "ReconciledFlag": "Y", "ReconciledDate": "YYYY-MM-DD",
--     "StatementId": X, "StmtLineId": Y }
-- (status is NOT updated during bank reconciliation)
