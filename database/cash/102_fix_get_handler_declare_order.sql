-- =============================================================================
-- PATCH 102: Corrected GET /cash/externaltransactions handler
--
-- SUPERSEDES patches 100 and 101.
--
-- FIX vs patch 101:
--   Oracle PL/SQL requires that nested subprograms (FUNCTION / PROCEDURE)
--   appear LAST in the DECLARE section — after all variables, types, and
--   cursors.  Patch 101 declared jstr()/jnum() before the cursor, which
--   caused: ORA-06550 PLS-00103: Encountered the symbol "CURSOR" when
--   expecting one of the following: begin function pragma procedure
--
--   This patch moves the CURSOR declaration above the nested functions.
--
-- HOW TO RUN:
...
BEGIN
    ...
    ORDS.DEFINE_HANDLER(
        ...
        p_items_per_page => 0,
        p_mimes_allowed  => '',
        p_comments       => 'Query external cash transactions (patch 102: correct DECLARE order)',
        p_source         => q'#
DECLARE
    ...
    -- ── Cursor declared before nested functions (Oracle PL/SQL requirement) ──
    CURSOR c_txns IS
    ...
    -- ── Nested functions declared last (Oracle PL/SQL requirement) ───────────
...
BEGIN
    -- Total count (same WHERE clause, no pagination)
    SELECT COUNT(*)
    INTO   l_total
    FROM   RR_EXTERNAL_CASH_TRANSACTIONS
    WHERE  (l_ext_id      IS NULL OR EXTERNAL_TRANSACTION_ID = l_ext_id)
...
#'
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('GET /cash/externaltransactions redeployed (patch 102: correct DECLARE order).');
END;
/
