-- ============================================================
-- Patch 103: Fix external transactions GET handler
--
-- Problem: TO_CHAR(AMOUNT) uses session NLS_NUMERIC_CHARACTERS
-- which may produce "190,400" (comma as thousands separator)
-- making the JSON invalid — causes "Unterminated fractional number".
--
-- Fix: Use TO_CHAR(AMOUNT, 'TM9') which always produces a plain
-- decimal number regardless of NLS settings.
--
...
DECLARE
    ...
    -- Always produce a plain decimal number (no thousands separator, no NLS effects)
    FUNCTION jnum(p IN NUMBER) RETURN VARCHAR2 IS
        v VARCHAR2(100);
    BEGIN
        IF p IS NULL THEN RETURN 'null'; END IF;
        v := TO_CHAR(p, 'TM9');
        -- TM9 can produce trailing dot for whole numbers (e.g. -1255.) — strip it
        RETURN RTRIM(v, '.');
    END jnum;
...
BEGIN
    ...
    LOOP
        ...
        DBMS_LOB.APPEND(v_clob, TO_CLOB('{'));
        DBMS_LOB.APPEND(v_clob, TO_CLOB('"externalTransactionId":' || jnum(r.EXTERNAL_TRANSACTION_ID)));
        DBMS_LOB.APPEND(v_clob, TO_CLOB(',"transactionId":'        || jnum(r.TRANSACTION_ID)));
        ...
        DBMS_LOB.APPEND(v_clob, TO_CLOB(',"transferId":'           || jnum(r.TRANSFER_ID)));
        ...
        DBMS_LOB.APPEND(v_clob, TO_CLOB(',"payeeId":'              || jnum(r.PAYEE_ID)));
...
END;
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('cash/externaltransactions GET handler updated');
END;
/
