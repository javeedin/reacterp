-- ============================================================
-- PATCH 64: Fix cash/externaltransactions GET handler
--
-- Problems fixed:
--   1. business_unit filter used exact match (=) — changed to LIKE
--      so partial / case-insensitive BU names still filter correctly.
--   2. No bank_account guard — without it the endpoint returns every
--      record in the table.  Added: return empty set when bank_account
--      is not supplied, matching the same guard used by banktxn-lines
...
BEGIN
    ...
    ORDS.DEFINE_HANDLER(
        ...
        p_items_per_page => 0,
        p_mimes_allowed  => '',
        p_comments       => 'Query external cash transactions — requires bank_account; business_unit uses LIKE',
        p_source         => q'[
DECLARE
    ...
    CURSOR c_txns IS
        SELECT EXTERNAL_TRANSACTION_ID,
         ...
         WHERE l_bank_acct IS NOT NULL   -- require bank_account; return nothing if omitted
