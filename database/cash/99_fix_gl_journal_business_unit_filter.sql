-- ============================================================
-- PATCH 99: Fix GL_JOURNAL rows excluded by business_unit filter
--
-- Problem: GL_JOURNAL rows in RR_V_BANK_RECON_SYSTXNS have
-- NULL BUSINESS_UNIT (the GL lines table has no direct BU).
-- When the caller passes business_unit=BUIMERC CORP_DIFC_INVST,
-- the WHERE clause:
--   UPPER(BUSINESS_UNIT) LIKE '%...'
-- evaluates to NULL (not TRUE) for those rows, silently
-- excluding all GL journal lines from the result.
--
-- Fix: when SOURCE = 'GL_JOURNAL' and BUSINESS_UNIT IS NULL,
-- bypass the business unit filter (include the row regardless).
--
-- HOW TO RUN:
...
BEGIN
    ...
    ORDS.DEFINE_HANDLER(
        ...
        p_items_per_page => 0,
        p_mimes_allowed  => '',
        p_comments       => 'Unified system transactions — AP + External + Bank Transfers + GL Journals. Auto-resolves cash_account. GL_JOURNAL rows bypass business_unit filter when BU is null.',
        p_source         => q'[
DECLARE
    ...
    CURSOR c_data IS
        ...
        WHERE (
            ...
            OR
            -- GL Journal: filter by cash account combination (passed directly)
            (SOURCE = 'GL_JOURNAL'
             AND l_cash_acct IS NOT NULL
             AND ACCOUNT_COMBINATION = l_cash_acct)
        )
          AND (l_source       IS NULL OR SOURCE = l_source)
          -- Business unit: GL_JOURNAL rows have NULL BUSINESS_UNIT so bypass
          -- the filter for them — the account combination already scopes them.
