-- ============================================================
-- PATCH 93: Derive STATUS from RECONCILED_FLAG in GET handler
--
-- Problem:
--   Bank Recon updates RECONCILED_FLAG = 'Y' but the STATUS
--   column on RR_EXTERNAL_CASH_TRANSACTIONS may still hold the
--   old Fusion value ('UNR'). The GET handler returned STATUS
--   as-is, so reconciled transactions still showed Unreconciled
--   in Manage External Transactions.
--
-- Fix:
--   In the SELECT cursor, compute effective status as:
--     CASE WHEN RECONCILED_FLAG = 'Y' THEN 'REC' ELSE NVL(STATUS, 'UNR') END
--   This derives the correct status from RECONCILED_FLAG without
--   relying on the STATUS column being updated by bank recon.
--
-- HOW TO RUN:
...
BEGIN
  ...
  ORDS.DEFINE_HANDLER(
    ...
    p_items_per_page => 0,
    p_mimes_allowed  => '',
    p_comments       => 'Query external cash transactions — STATUS derived from RECONCILED_FLAG',
    p_source         => q'[
DECLARE
    ...
    CURSOR c_txns IS
        SELECT EXTERNAL_TRANSACTION_ID,
               ...
               REFERENCE_TEXT,
               SOURCE,
               -- Derive effective status from RECONCILED_FLAG so that
               -- bank-recon reconciled rows show REC even if STATUS was not updated
