-- ============================================================
-- PATCH 67: Fix RR_V_BANK_RECON_SYSTXNS — BANK_TRANSFER branch
--
-- Problem:
--   The view created in patch 65 filters GL journal lines with
--   WHERE l.REFERENCE3 = 'BANK_ASSET', but REFERENCE3 is blank
--   in existing journal lines — so zero bank transfer rows are
--   returned.
--
-- Fix:
--   Use REFERENCE5 IN ('BANKTFR-DISBURSE','BANKTFR-RECEIPT')
--   AND REFERENCE7 IS NOT NULL instead.
--
--   REFERENCE5 holds the event type (reliably populated).
--   REFERENCE7 holds the bank account name and is only set on
--   the bank asset lines, not on the clearing/offset lines —
--   so this combination correctly identifies the lines we want.
--
-- HOW TO RUN:
--   APEX SQL Workshop → SQL Commands
--   Run the CREATE OR REPLACE VIEW statement below (no BEGIN/END).
-- ============================================================
...
UNION ALL
    ...
    -- ── Bank Transfers (GL Journal Lines — bank asset lines only) ────────────
