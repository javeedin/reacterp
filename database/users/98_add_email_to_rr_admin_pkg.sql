-- =============================================================================
-- PATCH 98: Add EMAIL to RR_USER_ACCOUNTS + update RR_ADMIN_PKG body
--
-- HOW TO RUN in APEX SQL Workshop → SQL Commands:
--   1. Run the ALTER TABLE block first
--   2. Run the full CREATE OR REPLACE PACKAGE BODY block
-- =============================================================================

-- ── Step 1: Add EMAIL column (skips safely if already exists) ────────────────
BEGIN
...
-- ── Step 2: Rebuild package body with EMAIL in GET_USERS, CREATE_USER, UPDATE_USER
CREATE OR REPLACE PACKAGE BODY RR_ADMIN_PKG AS
  ...
  BEGIN
    ...
    LOOP
      ...
      v_first := FALSE;
      v_row := '{"module_code":"' || r.MODULE_CODE || '",'
