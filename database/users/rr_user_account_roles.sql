...
BEGIN
    ...
    USING (
        SELECT
            ...
            jt.ROLE_CODE,
            jt.CREATED_BY,
            TO_TIMESTAMP(REPLACE(jt.CREATION_DATE, 'T', ' '), 'YYYY-MM-DD HH24:MI:SS.FF TZH:TZM') AS CREATION_DATE,
            jt.LAST_UPDATED_BY,
            TO_TIMESTAMP(REPLACE(jt.LAST_UPDATE_DATE, 'T', ' '), 'YYYY-MM-DD HH24:MI:SS.FF TZH:TZM') AS LAST_UPDATE_DATE
        FROM JSON_TABLE(
            p_json_data,
            '$.items[*]'
            COLUMNS (
