-- =============================================================================
-- PATCH 20: Fix APPROVED_BY → APPROVAL_APPROVER_NAME in both approval handlers
--
-- PROBLEM:
--   Patch 19 re-introduced APPROVED_BY in the CASH branch of both:
--     • PUT approvals/email-replies/:id/process  (Step 1)
--     • PUT approvals/requests/:id/status        (Step 2)
--   RR_EXTERNAL_CASH_TRANSACTIONS has no APPROVED_BY column — the correct
--   column (added in a prior migration) is APPROVAL_APPROVER_NAME.
--   This causes HTTP 555: ORA-00904: "APPROVED_BY": invalid identifier
--   whenever an in-app approval or email-reply approval touches a CASH request.
--
-- FIX:
--   Redeploy both handlers replacing APPROVED_BY with APPROVAL_APPROVER_NAME
--   in every UPDATE against RR_EXTERNAL_CASH_TRANSACTIONS.
--
-- HOW TO RUN:
...
BEGIN
    ...
    -- FIX: use APPROVAL_APPROVER_NAME (not APPROVED_BY)
...
#'
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('PUT approvals/email-replies/:id/process fixed (patch 20: APPROVAL_APPROVER_NAME).');
END;
/

-- ── Step 2: PUT approvals/requests/:id/status ────────────────────────────────
BEGIN
    BEGIN
        ORDS.DELETE_HANDLER(
            p_module_name => 'reerp',
...
#'
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('PUT approvals/requests/:id/status fixed (patch 20: APPROVAL_APPROVER_NAME).');
END;
/
