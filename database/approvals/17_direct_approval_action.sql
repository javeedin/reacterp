-- =============================================================================
-- PATCH 17: Direct Approval Action endpoint
--
-- PURPOSE:
--   Replaces the broken POST approvals/requests/:id/action handler (PLS-00382)
--   with a simple PUT endpoint that directly updates the approval status.
--   No complex orchestration — used when approving/rejecting from inside the app.
--
-- NEW ENDPOINT:
--   PUT /ords/reerp/approvals/requests/:id/status
--   Body: { "status": "APPROVED"|"REJECTED"|"CANCELLED"|"RECALLED",
--            "actorName": "...", "comments": "..." }
--
-- HOW TO RUN:
...
END;
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('PUT approvals/requests/:id/status registered.');
END;
/
